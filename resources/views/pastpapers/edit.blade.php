...
@section('content')
    <div class="container-fluid">
        <div class="card">
            ...
            <form action="{{ route('past-papers.update', encrypt($paper->id)) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <select name="category" id="category" required
                                ...
                                @foreach ($categories as $category)
                                    <option {{ $paper->past_paper_category_id == $category->id ? 'selected' : '' }}
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="name" id="name" placeholder="Enter topic name" required
                                value="{{ $paper->name }}" class="form-control @error('name') is-invalid @enderror">
                            @error('name')
                                <span role="alert">
                                    <strong class="text-danger">
                                        {{ $message }}
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="description" id="description" placeholder="Enter topic description"
                                required value="{{ $paper->description }}"
                        ...
                        <div class="form-group col-md-6">
                            <label for="file">File (Update Pdf)</label>
                            <input accept=".pdf" type="file" name="file" id="file"
                                placeholder="Enter topic file" value="{{ old('file') }}"
